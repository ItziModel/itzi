...
def solve_q_loop2(
        np.ndarray[DTYPE_t, ndim=2] arr_z0, np.ndarray[DTYPE_t, ndim=2] arr_zup,
        np.ndarray[DTYPE_t, ndim=2] arr_n0, np.ndarray[DTYPE_t, ndim=2] arr_nup,
        np.ndarray[DTYPE_t, ndim=2] arr_h0, np.ndarray[DTYPE_t, ndim=2] arr_hup,
        np.ndarray[DTYPE_t, ndim=2] arrp_qw,
        np.ndarray[DTYPE_t, ndim=2] arrp_qn,
        np.ndarray[DTYPE_t, ndim=2] arr_qw_new,
        np.ndarray[DTYPE_t, ndim=2] arr_qn_new,
...
#~             arrp_qw.dtype == arrp_qn.dtype == DTYPE
    ...
    for r in xrange(rmax):
        for c in xrange(cmax):
            ...
            q_vect = sqrt(qy_avg*qy_avg + q0*q0)
            # calculate flow
            n = 0.5 * (arr_n0[r, c] + arr_nup[r, c-1])
            qup = arrp_qw[rq, cq-1]
            qdown = arrp_qw[rq, cq+1]
            if hf <= hf_min:
                qw_new = 0
