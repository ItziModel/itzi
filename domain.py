...
class SuperficialSimulation(object):
    ...
    def solve_q(self):
        ...
        z_i0 = self.dom.get('z')[self.s_i_0]
        z_i1 = self.dom.get('z')[self.s_i_1]
        z_j0 = self.dom.get('z')[self.s_j_0]
        z_j1 = self.dom.get('z')[self.s_j_1]
        assert z_i0.shape == z_i1.shape
        assert z_j0.shape == z_j1.shape

        h_i0 = self.dom.get('h')[self.s_i_0]
        h_i1 = self.dom.get('h')[self.s_i_1]
        h_j0 = self.dom.get('h')[self.s_j_0]
        h_j1 = self.dom.get('h')[self.s_j_1]
        assert h_i0.shape == h_i1.shape
        assert h_j0.shape == h_j1.shape

        n_i0 = self.dom.get('n')[self.s_i_0]
        n_i1 = self.dom.get('n')[self.s_i_1]
        n_j0 = self.dom.get('n')[self.s_j_0]
        n_j1 = self.dom.get('n')[self.s_j_1]
        assert n_i0.shape == n_i1.shape
        assert n_j0.shape == n_j1.shape

        # flow depths
        hf_i = self.dom.get('hfe')[self.s_i_0]
        hf_j = self.dom.get('hfs')[self.s_j_0]

        # values in the Y dim, used to calculate qnorm in X dimension
        arr_qs_i_j = self.dom.get('qs')
        arr_qs_i_ju = self.dom.get_padded('qs')[self.su, self.ss]
        arr_qs_id_j = self.dom.get_padded('qs')[self.ss, self.sd]
        arr_qs_id_ju = self.dom.get_padded('qs')[self.su, self.sd]
        ...
        arr_qe_i_j = self.dom.get('qe')
        arr_qe_iu_j = self.dom.get_padded('qe')[self.ss, self.su]
        arr_qe_i_jd = self.dom.get_padded('qe')[self.sd, self.ss]
        arr_qe_iu_jd = self.dom.get_padded('qe')[self.sd, self.su]
        ...
        q_i0 = self.dom.get('qe')[self.s_i_0]
        q_i1 = self.dom.get('qe')[self.s_i_1]
        q_j0 = self.dom.get('qs')[self.s_j_0]
        q_j1 = self.dom.get('qs')[self.s_j_1]
        # Uses padded array to get boundary flow
        q_im1 = self.dom.get_padded('qe')[s_i_m1]
        q_jm1 = self.dom.get_padded('qs')[s_j_m1]
        assert q_i0.shape == q_i1.shape == q_im1.shape
        assert q_j0.shape == q_j1.shape == q_jm1.shape

        q_i0_new = self.dom.get('qe_new')[self.s_i_0]
        q_j0_new = self.dom.get('qs_new')[self.s_j_0]
        ...
        flow.solve_q(arr_dir=self.dom.get('dire'),
        ...
        assert q_j0.shape == q_j1.shape == q_jm1.shape == q_j0_new.shape
        flow.solve_q(arr_dir=self.dom.get('dirs'),
