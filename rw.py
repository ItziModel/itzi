...
def write_sim_data(opt_h, opt_wse, h_grid_np1, z_grid, can_ovr,
    ...
    if opt_h or opt_wse:
        grass.info(_("Printing results, simulation time: %.1f ") %
                 round(sim_clock,1))
        if opt_h:
            # define new map name
            ...
            grass.info(_("Writting water depth map <%s> ") % depth_map_name)
        ...
        if opt_wse:
            ...
            grass.info(_("Writting water surface elevation map <%s> ") % wse_map_name)
            # add map name to the map list
...
def write_raster(raster_name, arr, can_ovr):
    ...
    if can_ovr == True and raster.RasterRow(raster_name).exist() == True:
        ...
        grass.verbose(_("Removing raster map %s") % raster_name)
